...
@Keep
public class StartIoMediationAdapter extends MediationAdapterBase implements MaxInterstitialAdapter, MaxRewardedInterstitialAdapter, MaxRewardedAdapter, MaxAdViewAdapter, MaxNativeAdAdapter, MaxSignalProvider {
    private static final String LOG_TAG = StartIoMediationAdapter.class.getSimpleName();
    ...
    @Keep
    public StartIoMediationAdapter(AppLovinSdk sdk) {
        super(sdk);
    ...
    private boolean isInvalidAdapter(@NonNull MaxAdapterParameters parameters) {
        ...
        String adapterClassName = serverParameters.getString("adapter_class");
        if (adapterClassName == null) {
            return true;
        }

        try {
            if (StartIoMediationAdapter.class.isAssignableFrom(Class.forName(adapterClassName))) {
                // adapter is valid
                return false;
            }
        } catch (Throwable ex) {
    ...
    private boolean ensureInitialized(@NonNull Context context, @NonNull MaxAdapterParameters parameters, @Nullable OnCompletionListener listener) {
        ...
        synchronized (StartIoMediationAdapter.class) {
